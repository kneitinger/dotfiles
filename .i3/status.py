...
if "FreeBSD" in uname[0]:
    stat.register("shell", command="~/.i3/status_scripts/freebsd-batt.py 0", interval=2)
...
if which("pacman") is not None:
    stat.register("shell", command="~/.i3/status_scripts/update-stat.sh", interval=113)

if which("jack") is not None:
    stat.register("shell", command="~/.i3/status_scripts/jack-stat.sh", interval=7)

if "Linux" in uname[0]:
    stat.register("shell", command="~/.i3/status_scripts/spotify_song.sh", interval=4)
